...
BUILD_DIR = PROJECT / "var" / "market_value_chart_build_v4"
BACKUP_DIR = (
    PROJECT
    / "var"
    / (
...
def main() -> int:
    ...
    try:
        ...
        validated_pages: list[Path] = []

        for player, page in detected:
            built_page = validate_page(player, page)
            validated_pages.append(built_page)
            print(
                "Validated: "
                + player["name"]
                + " -> "
                + str(built_page.relative_to(BUILD_DIR))
            )

        if len(validated_pages) != 8:
            raise RuntimeError(
                "Expected 8 visible charts, found "
                + str(len(validated_pages))
            )

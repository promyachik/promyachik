...
BUILD_DIR = PROJECT / "var" / "market_value_chart_build_v2"
BACKUP_DIR = (
    PROJECT
    / "var"
    / (
...
def expected_built_page(source_page: Path) -> Path:
    relative = source_page.relative_to(
        PROJECT / "content"
    )
    ...
    if url_match:
        url_path = url_match.group(1).strip().strip("/")
        return BUILD_DIR / url_path / "index.html"
    ...
    if source_page.name.casefold() == "index.md":
        output_relative = relative.parent
    else:
    ...
    if slug_match:
        output_relative = (
            ...
            / slug_match.group(1).strip()
        )

    return BUILD_DIR / output_relative / "index.html"
...
def validate_page(
    ...
    built_page = expected_built_page(source_page)

    if not built_page.exists():
        raise RuntimeError(
            "Hugo did not build the detected page for "
            + player["name"]
            + ": "
            + str(built_page)
        )
    ...
    if 'class="player-market-chart"' not in html:
        raise RuntimeError(
            "Market value chart is missing for "
            + player["name"]
        )

    if "ДИНАМИКА СТОИМОСТИ" not in html:
        raise RuntimeError(
            "Chart heading is missing for "
            + player["name"]
        )
...
def main() -> int:
    ...
    candidate_pages = [
        ...
        if page.name.casefold() != "_index.md"
    ]

...
BACKUP_DIR = PROJECT / f"_backup_promyachik_233_before_force_align_value_labels_{timestamp}"
REPORT = PROJECT / "var" / "promyachik_233_force_align_value_labels_to_points_report.txt"

commands = []
changed = []

CSS_START = "/* PROMYACHIK 233 FORCE ALIGN VALUE LABELS START */"
CSS_END = "/* PROMYACHIK 233 FORCE ALIGN VALUE LABELS END */"

PARTIAL_NAME = "promyachik-force-align-value-labels-233.html"
PARTIAL_INCLUDE = '{{ partial "promyachik-force-align-value-labels-233.html" . }}'

CSS_BLOCK = '''
/* PROMYACHIK 233 FORCE ALIGN VALUE LABELS START */

/*
   Подписи дата+цена под графиком стоимости ставятся по X-точкам графика.
   Не центрирование всего ряда, а конкретная привязка каждого label-блока к своей точке.
*/

body.transfer-page .promyachik-value-label-row-233 {
...
body.transfer-page .promyachik-value-label-item-233 {
...
body.transfer-page .promyachik-value-label-item-233 span,
body.transfer-page .promyachik-value-label-item-233 strong,
body.transfer-page .promyachik-value-label-item-233 em {
    display: block !important;
    width: 100% !important;
    text-align: center !important;
}

/* PROMYACHIK 233 FORCE ALIGN VALUE LABELS END */
'''
...
def update_style():
    ...
    text = text.rstrip() + "\n\n" + CSS_BLOCK.strip() + "\n"
    write(style, text, "add force alignment CSS for value labels")

def install_partial():
    copy_file(
        RESTORE / "layouts" / "partials" / PARTIAL_NAME,
        PROJECT / "layouts" / "partials" / PARTIAL_NAME,
        "install force alignment JS partial"
    )

def cleanup_old_includes(text: str) -> str:
    old_includes = [
...
def ensure_include_in_file(path: Path, label: str):
    if not path.exists():
        return

    text = read(path)
    text = cleanup_old_includes(text)

    if "</body>" in text:
        text = text.replace("</body>", PARTIAL_INCLUDE + "\n</body>", 1)
    else:
        text = text.rstrip() + "\n" + PARTIAL_INCLUDE + "\n"

    write(path, text, label)

def include_partial():
    ensure_include_in_file(
        PROJECT / "layouts" / "transfers" / "single.html",
        "include 233 alignment partial directly in transfer page template"
    )

    # Also keep it in baseof for any other transfer render path.
    ensure_include_in_file(
        PROJECT / "layouts" / "_default" / "baseof.html",
        "include 233 alignment partial in baseof fallback"
    )
    ...
    for page in list(public_transfers.glob("*/index.html"))[:30]:
        ...
        for token in tokens:
            ...
            if idx != -1:
                ...
                fragments.append((rel(page), token, fragment[:680]))
                break
    return fragments[:10]
...
def main():
    ...
    try:
        ...
        transfer_template = read(PROJECT / "layouts" / "transfers" / "single.html") if (PROJECT / "layouts" / "transfers" / "single.html").exists() else ""
        ...
        for page in public_transfer_pages[:8]:
            public_sample += read(page)[:7000]

        checks = {
            "hugo_exit_code": hugo.returncode,
            "style_has_233_css_block": CSS_START in style_text and CSS_END in style_text,
            ...
            "old_232_block_removed": "PROMYACHIK 232 ALIGN VALUE LABELS TO POINTS START" not in style_text,
            "transfer_template_includes_233_partial": PARTIAL_INCLUDE in transfer_template,
            "baseof_includes_233_partial": PARTIAL_INCLUDE in baseof_text,
            "public_transfer_pages_found": len(public_transfer_pages),
            "public_has_233_script": "__promyachikForceAlignValueLabels233Ready" in public_sample,
        ...
        ok = (
            hugo.returncode == 0
            and checks["style_has_233_css_block"]
            and checks["transfer_template_includes_233_partial"]
            and checks["public_has_233_script"]
    ...
    lines = []
    lines.append("PROMYACHIK 233 - FORCE ALIGN VALUE LABELS TO CHART POINTS")
    lines.append("=" * 100)
    ...
    lines.append("- Positions date/value labels under dynamic player value chart using actual chart-point X coordinates.")
    lines.append("")
    lines.append("BACKUP")

...
def validate_source(js_text: str, css_text: str) -> None:
    ...
    if found_css:
        raise RuntimeError(
            "Old last-logo inset values still remain: "
            + ", ".join(found_css)
        )

    if css_text.count(CSS_START) != 1:
        raise RuntimeError(
            "The V41 chart CSS block appears an unexpected number of times."
        )
...
def validate_build() -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix="profutbik_chart_clean_v41_"
    ) as temporary:
...
def main() -> int:
    ...
    try:
        print()
        print(
            "STEP 1 OF 3: centering the final logo "
            "over the final transfer point..."
        )
        ...
        print(
            "STEP 2 OF 3: removing the yellow chart fill "
            "under the line..."
        )
        ...
        print()
        print("DONE")
        print("FINAL LOGO IS CENTERED OVER THE LAST POINT")
        print("YELLOW FILLED TRAIL UNDER THE LINE IS REMOVED")
        print("LINE, POINTS, LABELS AND PRICES ARE PRESERVED")

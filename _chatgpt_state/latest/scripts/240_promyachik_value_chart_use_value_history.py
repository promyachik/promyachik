...
BACKUP_DIR = PROJECT / f"_backup_promyachik_240_before_value_history_chart_{timestamp}"
REPORT = PROJECT / "var" / "promyachik_240_value_chart_use_value_history_report.txt"
...
CSS_START = "/* PROMYACHIK 240 VALUE HISTORY CHART START */"
CSS_END = "/* PROMYACHIK 240 VALUE HISTORY CHART END */"
...
CSS_BLOCK = '''
/* PROMYACHIK 240 VALUE HISTORY CHART START */

/*
   Исправление причины, почему 239 не появился:
   страницы используют .Params.value_history, а не только .Params.market_value_chart.
   График строится из market_value_chart.points, а если их нет — из value_history.
   У каждой точки одна SVG-ось:
   group translate(X 0), dot cx=0, date x=0, value x=0.
*/

body.transfer-page .pfb-value-chart-v240 {
    width: 100% !important;
...
body.transfer-page .pfb-value-chart-v240__head {
...
body.transfer-page .pfb-value-chart-v240__eyebrow {
...
body.transfer-page .pfb-value-chart-v240__title {
    margin: 0 !important;
...
body.transfer-page .pfb-value-chart-v240__subtitle {
...
body.transfer-page .pfb-value-chart-v240__current {
...
body.transfer-page .pfb-value-chart-v240__current strong {
    color: #ffffff !important;
...
body.transfer-page .pfb-value-chart-v240__svg {
...
body.transfer-page .pfb-value-chart-v240__background {
    fill: rgba(5, 7, 10, 0.78) !important;
}

body.transfer-page .pfb-value-chart-v240__grid {
    stroke: rgba(255, 255, 255, 0.08) !important;
    stroke-width: 2 !important;
}

body.transfer-page .pfb-value-chart-v240__line {
    stroke: url(#pfbValueLine240) !important;
}

body.transfer-page .pfb-value-chart-v240__logo {
    filter: drop-shadow(0 8px 12px rgba(0, 0, 0, 0.56)) !important;
}

body.transfer-page .pfb-value-chart-v240__fallback-circle {
    fill: rgba(212, 175, 55, 0.08) !important;
    stroke: #f2d374 !important;
    stroke-width: 6 !important;
}

body.transfer-page .pfb-value-chart-v240__dot-outer {
    fill: #090b0f !important;
    stroke: #f2d374 !important;
    stroke-width: 6 !important;
}

body.transfer-page .pfb-value-chart-v240__dot-inner {
    fill: #11151b !important;
    stroke: none !important;
}

body.transfer-page .pfb-value-chart-v240__date-label {
...
body.transfer-page .pfb-value-chart-v240__value-label {
...
body.transfer-page .pfb-value-chart-v240__note {
    margin: 8px 0 0 !important;
...
body.transfer-page .pfb-value-chart-v240__note a {
    color: #d8b34a !important;
...
/* PROMYACHIK 240 VALUE HISTORY CHART END */
'''
...
def replace_dynamic_chart():
    copy_file(
        RESTORE / "layouts" / "partials" / "transfer-player-market-value-chart.html",
        TARGET_DYNAMIC,
        "replace chart partial so it renders value_history when market_value_chart is absent"
    )
...
def cleanup_style():
    ...
    text = text.rstrip() + "\n\n" + CSS_BLOCK.strip() + "\n"
    write(STYLE, text, "remove failed chart CSS and add 240 value_history chart CSS")
...
def collect_fragments():
    ...
    for page in list(public_transfers.glob("*/index.html"))[:30]:
        text = read(page)
        if "pfb-value-chart-v240" in text:
            idx = text.find("pfb-value-chart-v240")
            fragments.append((rel(page), text[max(0, idx - 260): idx + 2200].replace("\n", " ")[:2400]))
    return fragments[:8]
...
def main():
    ...
    try:
        ...
        public_sample = ""
        for page in public_transfer_pages[:15]:
            public_sample += read(page)[:15000]
        ...
        checks = {
            ...
            "dynamic_partial_has_v240": "pfb-value-chart-v240" in dynamic_text,
            "dynamic_partial_uses_value_history": ".Params.value_history" in dynamic_text,
            "dynamic_partial_uses_market_chart_fallback": ".Params.market_value_chart" in dynamic_text,
            "dynamic_partial_left_pad_118": "{{- $leftPad := 118.0 -}}" in dynamic_text,
            "dynamic_partial_right_pad_118": "{{- $rightPad := 118.0 -}}" in dynamic_text,
            "dynamic_axis_group": 'class="pfb-value-chart-v240__axis"' in dynamic_text,
            ...
            "style_has_240_css": CSS_START in style_text and CSS_END in style_text,
            ...
            "public_has_240_chart": "pfb-value-chart-v240" in public_sample,
            "public_has_axis_group": "pfb-value-chart-v240__axis" in public_sample,
            "public_has_dot_cx_zero": 'cx="0"' in public_sample,
            "public_has_label_x_zero": 'x="0"' in public_sample,
            "public_has_value_history_labels": any(token in public_sample for token in ["€8M", "€15M", "€25M", "€30M", "€50M"]),
            "observed_240_fragments": len(fragments),
        }

        ok = (
            hugo.returncode == 0
            and checks["dynamic_partial_exists"]
            and checks["dynamic_partial_has_v240"]
            and checks["dynamic_partial_uses_value_history"]
    ...
    lines = []
    lines.append("PROMYACHIK 240 - VALUE CHART USE VALUE_HISTORY")
    lines.append("=" * 100)
    lines.append(f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append(f"Project dir: {PROJECT}")
    lines.append("")
    lines.append("RULE")
    lines.append("- Fixes the actual reason 239 did not render: pages use value_history, not only market_value_chart.")
    ...
    lines.append("- The visible chart partial now renders market_value_chart.points OR value_history.")
    ...
    if error_text:
        ...
        lines.append("")
    lines.append("OBSERVED 240 PUBLIC FRAGMENTS")
    if fragments:
